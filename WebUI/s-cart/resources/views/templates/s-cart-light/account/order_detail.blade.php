...
<div class="container">
  <div class="row">
    ...
    <div class="col-12 col-sm-12 col-md-9 min-height-37vh">
        <div class="row">
            <div class="col-sm-6"><h6 class="aside-title">{{ $title }}</h6></div>
            <div class="col-sm-6">
                <div class="btn-group float-right" style="margin-right: 10px;margin-bottom: 10px;border:1px solid #c5b5b5;">
                    @if ($order->shipping_status == 2)
                    <a class="btn btn-flat" title="Submit Ship" onclick="received_shipping({{ $order->tracking_number }})"><i class="fa fa-check"></i>&nbsp;&nbsp;<span class="hidden-xs">I Received</span></a>
                        @endif
                </div>
            </div>
        </div>
   ...
   <div class="col-sm-6">
    <table  class="table table-bordered">
        ...
        <tr><td>{{ trans('Tracking Number') }}:</td><td>{{ $order->tracking_number }}</td></tr>
...
@push('scripts')
    <script>
        function received_shipping(tracking_number) {
            var csrf = '{{ csrf_token() }}';
            $.ajax({
                url : '{{ route('soa.shipping.received') }}',
                type : "post",
